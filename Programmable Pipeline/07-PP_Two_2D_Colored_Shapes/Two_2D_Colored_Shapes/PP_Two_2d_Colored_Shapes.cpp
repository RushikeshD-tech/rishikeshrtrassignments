...
enum
{
	RAD_ATTRIBUTE_POSITION = 0,
	RAD_ATTRIBUTE_COLOR,
	RAD_ATTRIBUTE_NORMAL,
	RAD_ATTRIBUTE_TEXTURE0
};
...
GLuint gVaoTriangle;
GLuint gVaoSquare;
GLuint gVboTrianglePosition;
GLuint gVboTriangleColor;
GLuint gVboSquarePosition;
GLuint gVboSquareColor;
GLuint gMVPUniform;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("Rishikesh Dumbre : PP_Two_2D_Colored_Triangle"),
...
void initialize(void)
{
	...
	*/
	...
	glBindAttribLocation(gShaderProgramObject, RAD_ATTRIBUTE_POSITION, "vPosition");
	glBindAttribLocation(gShaderProgramObject, RAD_ATTRIBUTE_COLOR, "vColor");
	// Link the shader
	...
	glGenBuffers(1, &gVboTrianglePosition);					// Buffer to store vertex position
	glBindBuffer(GL_ARRAY_BUFFER, gVboTrianglePosition);	// Find that named object in memory
	glBufferData(GL_ARRAY_BUFFER, sizeof(triangleVertices), triangleVertices, GL_STATIC_DRAW);	// Takes data from CPU to GPU

	glVertexAttribPointer(RAD_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(RAD_ATTRIBUTE_POSITION);

	// Release the buffer for vertices:
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	// B. BUFFER BLOCK FOR COLORS:
	glGenBuffers(1, &gVboTriangleColor);					// Buffer to store vertex colors
	glBindBuffer(GL_ARRAY_BUFFER, gVboTriangleColor);		// Find that named object in memory
	glBufferData(GL_ARRAY_BUFFER, sizeof(triangleColors), triangleColors, GL_STATIC_DRAW);	// Takes data from CPU to GPU

	glVertexAttribPointer(RAD_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(RAD_ATTRIBUTE_COLOR);
	...
	// A. BUFFER BLOCK FOR VERTICES:
	glGenBuffers(1, &gVboSquarePosition);					// Buffer to store vertex position
	glBindBuffer(GL_ARRAY_BUFFER, gVboSquarePosition);		// Find that named object in memory
	...
	// B. BUFFER BLOCK FOR COLORS:
	glGenBuffers(1, &gVboSquareColor);					// Buffer to store vertex colors
	glBindBuffer(GL_ARRAY_BUFFER, gVboSquareColor);		// Find that named object in memory
...
void resize(int width, int height)
{
	...
	// gPerspective(left, right, bottom, top, near, far)
	// Co-ordinates written for glVertex3f() are relative to viewing volume of 100 X 100 
...
void uninitialize(void)
{
	...
	if (gVboTrianglePosition)
	{
		glDeleteBuffers(1, &gVboTrianglePosition);
		gVboTrianglePosition = 0;
	}

	// Destroy vbo for Triangle Color:
	if (gVboTriangleColor)
	{
		glDeleteBuffers(1, &gVboTriangleColor);
		gVboTriangleColor = 0;
	...
	if (gVboSquareColor)
	{
		glDeleteBuffers(1, &gVboSquareColor);
		gVboSquareColor = 0;
	}
