...
GLuint Texture_Stone;		//stone texture
GLuint Texture_Kundali;		//kundali texture
...
void Initialize(void)
{
	//Function Declaration
	void Resize(int, int);
	bool LoadGLTextures(GLuint*, TCHAR[]);  //Declare function to load the texture
	...
	LoadGLTextures(&Texture_Kundali, MAKEINTRESOURCE(STONE_BITMAP));
	LoadGLTextures(&Texture_Stone, MAKEINTRESOURCE(KUNDALI_BITMAP));
...
bool LoadGLTextures(GLuint *Texture, TCHAR ResourceId[])
{
	...
	//code
	hbitmap = (HBITMAP)LoadImage(GetModuleHandle(NULL), ResourceId, IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);  // Device Independent BITMAP
	...
	if (hbitmap)
	{
		...
		//Starts OPENGL Texture steps:
		glPixelStorei(GL_UNPACK_ALIGNMENT, 4);
		glGenTextures(1, Texture);
		glBindTexture(GL_TEXTURE_2D, *Texture);    // Target Point
	    //Setting Texture Parameters
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

		//Following Call Actually Push Data With Help Of Graphics Memory
		gluBuild2DMipmaps(GL_TEXTURE_2D, 3, bmp.bmWidth, bmp.bmHeight, GL_BGR_EXT, GL_UNSIGNED_BYTE, bmp.bmBits);  //glTexImage2D + glGenerateMipMap
