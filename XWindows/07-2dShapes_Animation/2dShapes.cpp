...
float angleTri = 0.0f;		//For traingle rotation
float angleRect = 0.0f;		//For square rotation
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay, gWindow, "2 2D rotating shapes");
...
void display(void)
{
	...
	glRotatef(angleTri,0.0f, 1.0f, 0.0f);	//Rotate the trianlge about y axis
	...
	//Reset the matrix for drawing the rectangle
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();

	glTranslatef(1.5f, 0.0f, -6.0f);
	glRotatef(angleRect,1.0f, 0.0f, 0.0f);	//Rotate the rectangle about x axis
	glBegin(GL_QUADS);
